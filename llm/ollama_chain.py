...
class OllamaState(TypedDict):
    """State definition for the Ollama LLM Chain"""
    input: str
...
class OllamaChain:
    """Ollama-based RAG chain with query enhancement, reranking, and retrieval"""

    def __init__(self, config: Optional[OllamaConfig] = None):
        """Initialize the Ollama Chain with optional custom configuration"""
    ...
    def _setup_llm(self) -> ChatOllama:
        ...
        logger.info(f"Using Ollama at {self.config.ollama_url} with model {self.config.ollama_model}")
        return ChatOllama(
    ...
    def _create_graph(self) -> StateGraph:
        """Create the processing graph with query enhancement and retrieval"""
        workflow = StateGraph(state_schema=OllamaState)
    ...
    def _enhance_query(self, state: OllamaState) -> str:
